...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ALAssetsGroup *group = (self.model.assetGroups)[[indexPath row]];
    [group setAssetsFilter:[ALAssetsFilter allPhotos]];
    
    NSString *groupName = [group valueForProperty:ALAssetsGroupPropertyName];
    NSInteger numAssets = [group numberOfAssets];
    
    NSString *title = [NSString stringWithFormat:@"%@ (%ld)", groupName, (long)numAssets];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(self.delegate != nil) {
        ...
        ALAssetsGroup *group = (self.model.assetGroups)[row];
