...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"cell offset %ld", (long)cell.dataOffset);
    // :D
    CGPoint cellPoint = [cell convertPoint:tapLocation fromView:self.tableView];
    NSInteger assetIndex = [cell assetOffsetFromPoint:cellPoint];
    if( (assetIndex>=0) && (assetIndex<[self.model.selectedGroupAssets count]) ) {
        ALAsset *asset = (self.model.selectedGroupAssets)[assetIndex];
