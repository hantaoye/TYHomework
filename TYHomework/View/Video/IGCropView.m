...
@interface IGCropView()<UIScrollViewDelegate, RSPlayerViewDelegate>
{
    CGSize _imageSize;
...
-(CGRect)getCropRegion
{
    if(self.alAsset)
    {
        ...
        else if([_type isEqualToString:ALAssetTypeVideo])
        {
            ...
            CGRect visibleRect = [self convertRect:self.bounds toView:self.videoPlayer];
...
+ (AVAsset *)cropVideo:(ALAsset *)alAsset withRegion:(CGRect)rect {
    AVURLAsset *asset = [AVURLAsset assetWithURL:alAsset.defaultRepresentation.url];
    return asset;
}
...
//    CGFloat sizeScale = self.zoomScale * [screen bounds].size.height / [screen bounds].size.width;
    return visibleRect = IGScaleRect(visibleRect, 1.0);
}
...
- (void)setAlAsset:(ALAsset *)asset
{
    ...
    else
    {
        ...
        self.videoPlayer = [[RSPlayerView alloc] init];
        self.videoPlayer.playerViewDelegate = self;
        self.videoPlayer.automaticallyShowStateView = NO;
        AVURLAsset *avasset = [AVURLAsset assetWithURL:asset.defaultRepresentation.url];
        ...
        self.videoPlayer.frame = CGRectMake(0, 0, size.width, size.height);
        
        [self addSubview:self.videoPlayer];
        self.videoPlayer.asset = avasset;
        [[self superview] addSubview:[self videoStateView]];
        
//        [self.videoPlayer play];
        [self configureForImageSize:self.videoPlayer.frame.size];
...
- (void)setFrame:(CGRect)frame {
    ...
    [[self videoStateView] setupImageViewFrame:[self frame]];
}
